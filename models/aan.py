...
class AAN(nn.Module):
    ...
    def __init__(self):
        ...
        if self.scale == 4 or self.scale == 8 or self.scale == 16:
    ...
    def forward(self, x):
        ...
        if self.scale == 1 or self.scale == 2 or self.scale == 3:
            fea = self.upconv1(F.interpolate(fea, scale_factor=self.scale, mode='nearest'))  # nearest/bilinear
            ...
            fea = self.upconv1(F.interpolate(fea, scale_factor=4, mode='nearest'))
            fea = self.lrelu(self.att1(fea))
            fea = self.lrelu(self.HRconv1(fea))
            fea = self.upconv2(F.interpolate(fea, scale_factor=4, mode='nearest'))
            fea = self.lrelu(self.att2(fea))
            fea = self.lrelu(self.HRconv2(fea))
